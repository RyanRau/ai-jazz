...
- **Chat** — persistent, multi-turn conversations, backed by
  ...
  `expandedContent` (`src/ChatHistoryList.tsx`) shows a New chat button, the
  5 most recent chats, and a "Show all chats" row — see
  `packages/PACKAGES.md` on why an item's own `expandedContent`, not a
  separate page-level panel, is where a page's short list of things to jump
  to belongs, and why it's capped rather than unbounded (the full list is
  `src/AllChatsPage.tsx`, one click away). The thread itself is
